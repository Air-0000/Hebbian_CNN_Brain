...
def train_standard_yolo(data_path, epochs=20, device='auto'):
    ...
    results = model.train(
        ...
        save=False,
        exist_ok=True,
        project='runs/opt_std',
        name='yolo',
        workers=4,  # 增加数据加载线程
        cache=True,  # 缓存数据到内存，加速训练
    )
...
def train_hebbian_yolo_optimized(data_path, epochs=20, device='auto'):
    """优化版Hebbian YOLOv8"""
    print("\n" + "="*60)
    print("训练 Optimized Hebbian YOLOv8")
    print("="*60)
    ...
    results = model.train(
        ...
        save=False,
        exist_ok=True,
        project='runs/opt_heb',
        name='yolo',
        workers=4,  # 增加数据加载线程
        cache=True,  # 缓存数据到内存，加速训练
        # Hebbian优化的参数（更合理）
        lr0=0.005,  # 稍微降低（不是0.0008）
        lrf=0.02,
        warmup_epochs=2,  # 减少预热
        mosaic=0.8,  # 保持正常增强
        close_mosaic=5,  # 后期减少增强
        weight_decay=0.0003,  # 轻微降低正则化
    )
    train_time = time.time() - start_time
    ...
    return {'model': 'Optimized Hebbian YOLOv8', 'mAP@0.5': map50, 'mAP@0.5:0.95': map, 'train_time': train_time}


def main():
    print("="*70)
    print("优化版 Hebbian YOLOv8 实验")
    print("="*70)
    ...
    # 优化Hebbian
    heb_results = train_hebbian_yolo_optimized(data_path, epochs)
    ...
    print(f"\n{'模型':<35} {'mAP@0.5':<12} {'mAP@0.5:0.95':<12} {'训练时间(s)':<12}")
    print("-" * 75)
    print(f"{std_results['model']:<35} {std_results['mAP@0.5']:.4f}{'':<7} {std_results['mAP@0.5:0.95']:.4f}{'':<7} {std_results['train_time']:.1f}")
    print(f"{heb_results['model']:<35} {heb_results['mAP@0.5']:.4f}{'':<7} {heb_results['mAP@0.5:0.95']:.4f}{'':<7} {heb_results['train_time']:.1f}")
    ...
    ax.set_ylabel('mAP')
    ax.set_title('Optimized Hebbian YOLOv8 Results')
    ax.set_xticks(x)
    ...
    plt.tight_layout()
    plt.savefig('optimized_hebbian_yolo.png', dpi=150, bbox_inches='tight')
    print("\n已保存: optimized_hebbian_yolo.png")

    df = pd.DataFrame([std_results, heb_results])
    df.to_excel('optimized_hebbian_results.xlsx', index=False)
    print("已保存: optimized_hebbian_results.xlsx")
