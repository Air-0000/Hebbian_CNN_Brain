"""
BioHebbianNet-DenseNet - 简化版DenseNet-40 + Hebbian
"""
...
print("="*70, flush=True)
print("BioHebbianNet-DenseNet-40", flush=True)
print("="*70, flush=True)
...
class DenseBlock(nn.Module):
    def __init__(self, layers, in_ch, growth=12, drop=0.2, hebbian=True):
        ...
        for i in range(layers):
            if hebbian:
                self.layers.append(DenseLayer(in_ch + i*growth, growth, drop))
            else:
...
# DenseNet-40: growth=12, 3个block各12层
class BioHebbianDenseNet(nn.Module):
...
class StandardDenseNet(nn.Module):
    def __init__(self, num_classes=10, growth=12, drop=0.0):
        super().__init__()
        ch = 16
...
def main():
    ...
    # BioHebbian DenseNet
    print("\n" + "="*70, flush=True)
    print("训练 BioHebbian-DenseNet-40", flush=True)
    print("="*70, flush=True)

    heb_model = BioHebbianDenseNet(num_classes=10, growth=12, drop=0.2)
    heb_acc, heb_hist, heb_state, heb_params = train_model(
        heb_model, "BioHebbian-DenseNet-40",
    ...
    torch.save({'model_state_dict': heb_state, 'best_acc': heb_acc, 'params': heb_params},
               'results/model_checkpoints/densenet_40_best.pth')
    print(f"已保存: results/model_checkpoints/densenet_40_best.pth", flush=True)

    # Standard DenseNet
    print("\n" + "="*70, flush=True)
    print("训练 Standard-DenseNet-40", flush=True)
    print("="*70, flush=True)

    std_model = StandardDenseNet(num_classes=10, growth=12, drop=0.0)
    ...
    torch.save({'model_state_dict': std_state, 'best_acc': std_acc, 'params': std_params},
               'results/model_checkpoints/standard_densenet_40_best.pth')

    # 结果
    baseline = {'DenseNet-40 (论文)': 94.5, 'ResNet-110': 94.2, 'BioHebbianNet v3': 92.35}

    print("\n" + "="*70, flush=True)
    print("DenseNet 实验结果", flush=True)
    print("="*70, flush=True)
    ...
    print(f"\n训练结果:", flush=True)
    print(f"  BioHebbian-DenseNet-40: {heb_acc:.2f}% ({heb_params/1e6:.2f}M)", flush=True)
    print(f"  Standard-DenseNet-40:  {std_acc:.2f}% ({std_params/1e6:.2f}M)", flush=True)

    df = pd.DataFrame({
        'Model': list(baseline.keys()) + ['BioHebbian-DenseNet-40', 'Standard-DenseNet-40'],
        'Accuracy (%)': list(baseline.values()) + [heb_acc, std_acc],
        'Parameters': ['1.0M', '1.7M', '4.82M', f'{heb_params/1e6:.2f}M', f'{std_params/1e6:.2f}M'],
    })
    df.to_excel('results/xlsx/densenet_results.xlsx', index=False)
    ...
    plt.tight_layout()
    plt.savefig('figures/densenet_training.png', dpi=150)
    print("\n已保存: figures/densenet_training.png", flush=True)
